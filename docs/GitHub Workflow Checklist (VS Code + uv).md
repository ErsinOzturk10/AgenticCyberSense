...
> Note: These steps help you catch locally the some extra checks that run in GitHub Actions (`build.yml`) after pushing.
...
* [ ] Do not continue until `ruff check` and `ruff format` complete successfully.
