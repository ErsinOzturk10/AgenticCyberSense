...
class TrafilaturaOllamaAgent:
    ...
    async def _fetch_with_playwright(self, url: str) -> tuple[str | None, str | None, list[str]]:
        try:
            ...
            logger.exception("Playwright fetch failed: %s", exc)
            return None, None, []
    ...
    async def _run_page(self, context: BrowserContext, url: str) -> tuple[str, str | None, list[str]]:
        ...
        try:
            ...
            try:
                ...
                logger.debug("Page goto failed attempts: %s; %s", exc, exc2)
                await page.close()
                return "", None, []
    ...
    def _fetch_static(self, url: str) -> tuple[str | None, str | None, list[str]]:
        try:
            ...
            logger.exception("Static fetch failed: %s", exc)
            return None, None, []
    ...
    def _build_result(
        ...
        if extraction_type == "threat_intel" and len(content) > 200:
            ...
            try:
                ...
                logger.exception("IOC extraction failed in _build_result: %s", exc)
            ...
            if domain in url_lower:
                for sel in selectors:
                    try:
                        ...
                        logger.debug("Selector wait failed: %s", exc)
                        continue
    ...
    async def _extract_links_with_playwright(self, page, base_url: str) -> list[str]:
        ...
        try:
            ...
            logger.exception("Failed to extract links with Playwright: %s", exc)
        return links
    ...
    def _extract_links_from_html(self, html: str, base_url: str) -> list[str]:
        ...
        try:
            ...
            logger.exception("Failed to extract links from HTML: %s", exc)
        return links[:200]
    ...
    def _extract_with_trafilatura(self, html: str, url: str) -> str:
        try:
            ...
            logger.exception("Trafilatura extraction failed: %s", exc)
            return ""
    ...
    def _fallback_extraction(self, html: str) -> str:
        ...
        try:
            ...
            logger.exception("Fallback extraction failed: %s", exc)
            return ""
    ...
    async def _extract_iocs_with_ollama(self, content: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception("IOC extraction failed: %s", exc)
