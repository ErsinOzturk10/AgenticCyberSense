...
# Number of failed sites to preview in logs
...
def print_statistics(
    ...
    """Print a summary of the crawl session."""
...
async def _watchdog(crawler: SmartDeepCrawler, crawl_task: asyncio.Task, inactivity_timeout: int) -> None:
    ...
    while not crawl_task.done():
        ...
        if elapsed > inactivity_timeout:
            logger.warning("Inactivity timeout (%ss) — cancelling site", inactivity_timeout)
            crawl_task.cancel()
            break
...
async def process_single_site(
    *,
    ...
    """Crawl a single site and store the result."""
    async with site_semaphore:
        ...
        try:
            ...
            if not results:
                ...
                logger.warning("INACTIVITY TIMEOUT [%d/%d]", idx, total)
...
async def main() -> None:
    """Run the full crawl cycle and update the RAG index when done."""
    logger.info("%s", "=" * 80)
    logger.info("CONCURRENT HASH-BASED CRAWLING SYSTEM v2")
    mode_label = "FORCE FULL CRAWL" if FORCE_FULL_CRAWL else "Hash-based incremental"
    ...
    est = len(urls) * 10 if FORCE_FULL_CRAWL else len(urls) * 3 // CONCURRENT_SITES
    logger.info("Estimated time   : ~%d minutes (concurrent=%d)", est, CONCURRENT_SITES)
    ...
    total_duration = (datetime.now(tz=UTC) - overall_start).seconds
    ...
    # Collect and log failed sites grouped by error message
    ...
    logger.info("\nCrawl completed — %d/%d sites successful (%d%%)", successful_sites, len(urls), pct)

    if not FORCE_FULL_CRAWL:
        logger.info("Next run will skip unchanged sites (hash-based).")
    else:
        logger.info("Set FORCE_FULL_CRAWL=False in config.py to enable hash optimization.")

    # ---------------------------------------------------------------
    # Crawl finished — update the RAG index with the new JSON output
    # ---------------------------------------------------------------
    logger.info("\nUpdating webcrawler RAG index...")
    try:
